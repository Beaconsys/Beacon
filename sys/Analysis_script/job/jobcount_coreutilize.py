...
def query_by_date(start, end):
    ...
    try:
        ...
        cursor = db.cursor()
        sql1 = (
            "select JOBID,CORE,STARTTIME,ENDTIME from JOB_log_all where STARTTIME > '"
            + start + "' AND ENDTIME < '" + end +
            "' AND QUEUE not like '%x86%' and STARTTIME<>'0000-00-00 00:00:00';"
        )
        sql2 = (
            "select JOBID,CORE,STARTTIME,ENDTIME from JOB_log_all where STARTTIME <= '"
            + start + "' AND ENDTIME >= '" + end +
        ...
        sql3 = (
            "select JOBID,CORE,STARTTIME,ENDTIME from JOB_log_all where STARTTIME < '"
            + start + "' AND ENDTIME <= '" + end + "' AND ENDTIME > '" + start
            +
            "'AND QUEUE not like '%x86%' and STARTTIME<>'0000-00-00 00:00:00';"
        )
        sql4 = (
            "select JOBID,CORE,STARTTIME,ENDTIME from JOB_log_all where STARTTIME >= '"
            + start + "' AND STARTTIME < '" + end + "' AND ENDTIME > '" + end +
            "'AND QUEUE not like '%x86%' and STARTTIME<>'0000-00-00 00:00:00';"
        )
        deal_first(sql1)
        deal_second(sql2)
        ...
        if type(item[3]) == types.NoneType:
            ...
            if en < st:
                if head <> 0:
                    ...
                    job_corehour[(st - start_timestamp) / 3600 -
                                 1] += (tail - head) / 3600.0 * core
                    continue
            elif en >= st:
                if head <> 0:
                    job_count[(st - start_timestamp) / 3600 - 1] += 1
                    job_corehour[(st - start_timestamp) / 3600 -
                                 1] += (3600 - head) / 3600.0 * core
                if tail <> 0:
                    job_count[(en - start_timestamp) / 3600] += 1
                    job_corehour[(en - start_timestamp) /
                                 3600] += tail / 3600.0 * core
...
if __name__ == "__main__":
    ...
    query_by_date(s, e)
    csvfile = file(
        './core_corehour_' + sys.argv[1] + '-' + sys.argv[3] + '.csv', 'wb')
