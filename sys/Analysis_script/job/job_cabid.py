...
def get_re(time1, time2):
    ...
    try:
        ...
        sql1 = ('select PROGRAM_NAME,NODELIST,RUNTIME,JOBID,STARTTIME,ENDTIME from JOB_log_all where ' + ite + ' and ' +
                ite1 + ' and ' + '( ' + item1 + ' or ' + item2 + ' or ' + item3 + ' ) order by PROGRAM_NAME')
        cursor.execute(sql1) 
...
#                    name_jobid[val[0]] = val[3]
            ...
            for key in name_cabid.keys():
                ...
                gio.sort()
                result.append([key, name_jobid[key], name_time[key], name_cabid[key], gio])
