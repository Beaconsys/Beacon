...
def workload():
    if request.method == 'POST':
        ...
        return render_template('workload.html',job_count=job_count,job_corehour=job_corehour,result=result)
    else:
...
def job_detail():
    if request.method == 'POST':
        ...
        if get_job_by_idd(jobid):
            ...
            #print nodelist
            return render_template('job_detail.html',job=job,nodelist=nodelist,nodecount=nodecount)
        else:
    ...
    if request.args.get('error_file'):
        ...
        if len(error_info)>=2:
            if 'None' in error_info[1]:
                error_info[1] = 'Sorry, the job is still RUNNING. Please wait until it finishes'
