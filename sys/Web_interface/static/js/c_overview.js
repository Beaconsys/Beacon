$("#submit").click(function() {
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            //$("#wavg").html(wavg);
            //$("#wsum").html(wsum);
            $('#img_ciobandwidth').attr('src', '/job_overview/'+jobid+"_ciobandwidth.png");
            $('#img_ciops').attr('src', '/job_overview/'+jobid+"_ciops.png");
            $('#img_cmetadata').attr('src', '/job_overview/'+jobid+"_cmetadata.png");
            $('#img_crdatadistribution').attr('src', '/job_overview/'+jobid+"_cread_datadistribution.png");
            $('#img_cwdatadistribution').attr('src', '/job_overview/'+jobid+"_cwrite_datadistribution.png");
            $('#img_cractiveprocess').attr('src', '/job_overview/'+jobid+"_cread_activeprocess.png");
            $('#img_cwactiveprocess').attr('src', '/job_overview/'+jobid+"_cwrite_activeprocess.png");
        },
        error : function() {
            if(!confirm("Opoos! Some error just happened! Do you want to view the detail error information?")){
                return false;
            }
            location.href = "/c_error?error_file="+jid+"_cdatadistribution.log";
            }
        })
    });
