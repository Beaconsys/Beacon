$("#submit").click(function() {
    ...
    $.ajax({
        url : "/draw_ciobandwidth",
        type : 'post',
        data : $('#form_ciobandwidth').serialize(),
        dataType : 'json',
        success : function(data) {
            ...
            $("#wavg").html(wavg);
            $("#wsum").html(wsum);
            $('#image').attr('src', '/image/'+jobid+"_ciobandwidth.png");
        },
        error : function() {
            if(!confirm("Opoos! Some error just happened! Do you want to view the detail error information?")){
                return false;
            }
            location.href = "/c_error?error_file="+jid+"_ciobandwidth.log";
            }
        })
    });
