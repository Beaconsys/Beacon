$("#submit").click(function() {
    ...
    $.ajax({
        url : "/draw_cactiveprocess",
        type : 'post',
        data : $('#form_cactiveprocess').serialize(),
        dataType : 'json',
        success : function(data) {
            ...
            //$("#wavg").html(wavg);
            //$("#wsum").html(wsum);
            $('#image_read_activeprocess').attr('src', '/image/'+jobid+"_cread_activeprocess.png");
            $('#image_write_activeprocess').attr('src', '/image/'+jobid+"_cwrite_activeprocess.png");
        },
        error : function() {
            if(!confirm("Opoos! Some error just happened! Do you want to view the detail error information?")){
                return false;
            }
            location.href = "/c_error?error_file="+jid+"_cactiveprocess.log";
            }
        })
    });
