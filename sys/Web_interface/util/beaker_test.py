# -*- coding: utf-8 -*-
import sys, datetime, time, elasticsearch1 as ES
...
mc = memcache.Client(['127.0.0.1:12001'], debug = 0)
#
cache_opts = {
    'cache.type' : 'dbm',
...
def query_ost(time_s, time_e):
    ...
    print 'Memcached caching...'
    mcs = time.time()
    for i in range(len(result)):
        mc.set(str(i), result[i])
    mce = time.time()
    print 'Memcached cache time : ' + str(round(mce - mcs, 2)) + ' s.'
