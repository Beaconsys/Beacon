...
def deal_queue(queues, daytime):
    ...
    time_a = datetime.datetime.strptime(daytime, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(hours=8)
    print queues[0]
    host_h = int(queues[0].split(' ')[0].split('.')[3]) - 17
    time_h = queues[0].split(' ')[1][1:] + ' ' + queues[0].split(' ')[2][0:-1]
    line_h = (datetime.datetime.strptime(time_h, "%Y-%m-%d %H:%M:%S") - time_a).seconds
    ...
    for line in range(1, file_line - 1):
        ...
        time_c = queues[line].split(' ')[1][1:] + ' ' + queues[line].split(' ')[2][0:-1]
...
def deal_latency(data_modify,daytime):
    ...
    for line in range(1, file_line):
        ...
        if op_h == op_c and time_h==time_c and host_h==host_c:
            ...
            for i in range(8):
                locals()['varc_' + str(i)] = int(data_modify[line-1].split(' ')[7 + i])

            if op_h == 'Read_wait':
                #print 'RW'
                for j in range(8):
                    matrix_rw[line_h][host_h * 8 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]
            elif op_h == 'Read_exe':
                #print "RE"
                for j in range(8):
                    matrix_re[line_h][host_h * 8 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Write_wait':
                #print "WW"
                for j in range(8):
                    matrix_ww[line_h][host_h * 8 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Write_exe':
                #print "WE"
                for j in range(8):
                    matrix_we[line_h][host_h * 8 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Meta_wait':
                #print "MW"
                for j in range(8):
                    matrix_mw[line_h][host_h * 8 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Meta_exe':
                #print "ME"
                for j in range(8):
                    matrix_me[line_h][host_h * 8 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]
            ...
            host_h= host_c
            time_h = time_c
            line_h = (datetime.datetime.strptime(time_c, "%Y-%m-%d %H:%M:%S") - time_a).seconds
    ...
    np.savetxt("./csv_data/read_wait"+finame+".csv", matrix_rw, fmt = "%d", delimiter = ",")
    np.savetxt("./csv_data/read_exe"+finame+".csv", matrix_re, fmt = "%d", delimiter = ",")
    np.savetxt("./csv_data/write_wait"+finame+".csv", matrix_ww, fmt = "%d", delimiter = ",")
    np.savetxt("./csv_data/write_exe"+finame+".csv", matrix_we, fmt = "%d", delimiter = ",")
    np.savetxt("./csv_data/meta_wait"+finame+".csv", matrix_mw, fmt = "%d", delimiter = ",")
    np.savetxt("./csv_data/meta_exe"+finame+".csv", matrix_me, fmt = "%d", delimiter = ",")
    ...
    for mon in range(2,3):
        ...
        for index_item in range(22,29):
            ...
            if index_item<10:
                ...
                daytime="2018-"+m+"-0"+str(index_item)+" "+"00:00:00"
                time_std="2018-"+m+"-0"+str(index_item)+" "+"20:00:00"
            else:
                index="2018."+m+"."+str(index_item)
                daytime="2018-"+m+"-"+str(index_item)+" "+"00:00:00"
                time_std="2018-"+m+"-"+str(index_item)+" "+"20:00:00"
            host_t=91
            print(index)
            print(daytime)
