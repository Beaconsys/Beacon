...
def deal_queue(queues, time_s, time_e):
    ...
    time_a = datetime.datetime.strptime(time_s, "%Y-%m-%d %H:%M:%S") + datetime.timedelta(hours=8)
...
def create_csv(data, time_s, time_e):
    ...
    for line in range(1, file_line - 1):
        ...
        op_next = data[line + 1].split(' ')[5] + '_' + data[line].split(' ')[6][0:-1]
        ...
        if op_next == op_c:
            ...
            time_c = data[line].split(' ')[1][1:] + ' ' + data[line].split(' ')[2][0:-1] 
            line_c = (datetime.datetime.strptime(time_c, "%Y-%m-%d %H:%M:%S") - time_a).seconds
            ...
            if op_c == 'Read_wait':
                ...
                for j in range(0,8):
                    matrix_rw[line_c][host_c * 9 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]
            elif op_h == 'Read_exe':
                #print "RE"
                matrix_re[line_c][host_c * 9] = int(host_c)
                for j in range(0,8):
                    matrix_re[line_c][host_c * 9 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Write_wait':
                #print "WW"
                matrix_ww[line_c][host_c * 9] = int(host_c)
                for j in range(0,8):
                    matrix_ww[line_c][host_c * 9 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Write_exe':
                #print "WE"
                matrix_we[line_c][host_c * 9] = int(host_c)
                for j in range(0,8):
                    matrix_we[line_c][host_c * 9 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Meta_wait':
                #print "MW"
                matrix_mw[line_c][host_c * 9] = int(host_c)
                for j in range(0,8):
                    matrix_mw[line_c][host_c * 9 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]   
            elif op_h == 'Meta_exe':
                #print "ME"
                matrix_me[line_c][host_c * 9] = int(host_c)
                for j in range(0,8):
                    matrix_me[line_c][host_c * 9 + j] = locals()['varc_' + str(j)] - locals()['varh_' + str(j)]
            
            #print "Updating the head" #OK
            op_h = op_next
            for a in range(0,8):
                locals()['varh_' + str(a)] = int(data[line + 1].split(' ')[7 + a])
