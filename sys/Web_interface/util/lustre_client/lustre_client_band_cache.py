...
parser = OptionParser()
parser.add_option("-d", "--draw", default = False, action = "store_true", help = "Draw the figures", dest = "draw")
parser.add_option("-t", "--trace", default = False, action = "store_true", help = "save trace", dest = "trace")
parser.add_option("-a", "--auto", default = False, action = "store_true", help = "run with automatic mode", dest = "auto")
parser.add_option("-o", "--old", default = False, action = "store_true", help = "deal old format data", dest = "old")
parser.add_option("-c", "--cache", default = False, action = "store_true", help = "deal cache result", dest = "cache")
parser.add_option("-b", "--band", default = False, action = "store_true", help = "deal band result", dest = "band")
            ...
            if host == host_p:
                ...
                if interval > 0 and interval < 1000:
                    for j in range(interval):
                        hit_list[band_number][t_p+j] += hit_value / 1024.0 * 4 / interval
                        miss_list[band_number][t_p+j] += miss_value / 1024.0 * 4 / interval
                        discard_list[band_number][t_p+j] += discard_value / 1024.0 * 4 / interval
                host_p = host
        ...
        for mon in range(month_s, month_e+1):
            ...
            for index_item in range(day_s, day_e+1):
                if index_item < 10:
                    ...
                    time_std = "2018-" + m + "-0" + str(index_item) + " " + "12:00:00"
                else:
                    index = "2018." + m + "." + str(index_item)
                    daytime = "2018-" + m + "-" + str(index_item) + " " + "00:00:00"
                    time_std = "2018-" + m + "-" + str(index_item) + " " + "12:00:00"
                print index
                print daytime
