...
def findjob(time1,time2):#RUNTIME cover time1 and time2
	conn=MySQLdb.connect(host='20.0.2.12',user='swqh',db='JOB',passwd='',port=3306)
...
#Modified by tyzhang. To maintain all the trace in mysql.
        sql1="select JOBID,JOB_NAME,PROGRAM_NAME,STARTTIME,ENDTIME,RUNTIME,CNC,CORE,NODELIST from "+database+" where jobid="+str(jobid)+" and RUNTIME<>0 order by PROGRAM_NAME"
		#sql1="select JOBID,USER,STATE,QUEUE,CNC,STARTTIME,ENDTIME,RUNTIME,PROGRAM_NAME,NODELIST from JOB_log where "+ite+" and "+item3+" order by PROGRAM_NAME"
		#print sql1
        cursor.execute(sql1)
        result1=cursor.fetchall()
...
#    print "shshshhhhhhhhhhhhhhhhhhhhhhhhhhh"
    try:
        ...
        if int(jobid)<=10000000:
            ...
            database="JOB_log"
#Modified by tyzhang. To maintain all the trace in mysql.
        sql1="select CNC from "+database+" where jobid="+str(jobid)+" and RUNTIME<>0 order by PROGRAM_NAME"
        cursor.execute(sql1)
        result1=cursor.fetchall()
        print result1[0][2:-3]
        conn.commit()
...
#    print "shshshhhhhhhhhhhhhhhhhhhhhhhhhhh"
	...
	print database
#Modified by tyzhang. To maintain all the trace in mysql.
        sql1="select JOBID,JOB_NAME,PROGRAM_NAME,STARTTIME,ENDTIME,RUNTIME,CNC,COREHOUR,NODELIST from "+database+" where jobid="+str(jobid)+" and RUNTIME<>0 order by PROGRAM_NAME"
		#sql1="select JOBID,USER,STATE,QUEUE,CNC,STARTTIME,ENDTIME,RUNTIME,PROGRAM_NAME,NODELIST from JOB_log where "+ite+" and "+item3+" order by PROGRAM_NAME"
		#print sql1
        cursor.execute(sql1)
        result1=cursor.fetchall()
        ...
        if len(re)<1:
            ...
            for val in re:
                ...
                node=val[8]
                node=node.split(",")
                resu.append([id,jname,pname,stime,etime,atime,cnc,corehour,node])
    except Exception as e:
        print e
        conn.rollback()
    print resu
...
#    print "shshshhhhhhhhhhhhhhhhhhhhhhhhhhh"
    try:
        ...
        if int(jobid)<=10000000:
            ...
            database="JOB_log"
#Modified by tyzhang. To maintain all the trace in mysql.
        sql1="select JOBID,JOB_NAME,PROGRAM_NAME,STARTTIME,ENDTIME,CNC,RUNTIME,COREHOUR,NODELIST from "+database+" where jobid="+str(jobid)+" and RUNTIME<>0 order by PROGRAM_NAME"
		#sql1="select JOBID,USER,STATE,QUEUE,CNC,STARTTIME,ENDTIME,RUNTIME,PROGRAM_NAME,NODELIST from JOB_log where "+ite+" and "+item3+" order by PROGRAM_NAME"
		#print sql1
        cursor.execute(sql1)
        result1=cursor.fetchall()
        ...
        if len(re)<1:
            ...
            for val in re:
                ...
                node=val[8]
                node=node.split(",")
                resu.append([id,jname,pname,stime,etime,cnc,runtime,corehour,node])
    except Exception as e:
        print e
        conn.rollback()
    return resu
