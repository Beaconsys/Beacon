...
def save_all(jobID, CNC, runtime, resultr_band, resultw_band, resultr_iops, resultw_iops, resultr_open, resultw_close, pe_r, pe_w, tmp_file_name):
    f = open(tmp_file_name, 'wb')
    for i in range(len(resultr_band)):
        if abs(resultr_band[i]) > sys.float_info.epsilon or abs(resultr_band[i]) > sys.float_info.epsilon or abs(resultr_iops[i]) > sys.float_info.epsilon or abs(resultw_iops[i]) > sys.float_info.epsilon or abs(resultr_open[i]) > sys.float_info.epsilon or abs(resultw_close[i]) > sys.float_info.epsilon or abs(pe_r[i]) > sys.float_info.epsilon or abs(pe_w[i]) > sys.float_info.epsilon:
...
def deal_csv(results_message, results_host, min_time, max_time, time1, time2):
    ...
    return (
     resultr, resultw, resultr_ops, resultw_ops, result_open, result_close, resultr_size, resultw_size, dictr, dictw)
...
def comput(time1, time2, iplist, cache_file):
    ...
    q0 = RangeQuery(qrange=ESRange('@timestamp', from_value=time1, to_value=time2))
...
def sum_IOBANDWIDTH(resultr, resultw):
    ...
    for i in range(len(resultr)):
        ...
        if abs(resultr[i]) > IOBW_affective or abs(resultw[i]) > IOBW_affective:
            count_resultrw += 1
    ...
    return (
     sum_resultr, sum_resultw, average_resultr, average_resultw, count_resultr, count_resultw, count_resultrw, average_resultr, average_resultw)
...
def sum_IOPS(IOPS_r, IOPS_w, IOBW_r, IOBW_w):
    ...
    for i in range(len(IOPS_r)):
        if abs(IOPS_r[i]) > sys.float_info.epsilon and IOBW_r[i] > IOBW_affective:
            count_IOPS_r += 1
            sum_IOPS_r += IOPS_r[i]
        if abs(IOPS_w[i]) > sys.float_info.epsilon and IOBW_w[i] > IOBW_affective:
            count_IOPS_w += 1
            sum_IOPS_w += IOPS_w[i]
        if abs(IOPS_r[i]) > sys.float_info.epsilon and IOBW_r[i] > IOBW_affective or abs(IOPS_w[i]) > sys.float_info.epsilon and IOBW_w[i] > IOBW_affective:
            count_IOPS_rw += 1
            count_IOPS_rw_all += 1
...
def sum_MDS(MDS_o, MDS_c):
    ...
    for i in range(len(MDS_o)):
        ...
        if abs(MDS_c[i]) > sys.float_info.epsilon:
            count_MDS_c += 1
            sum_MDS_c += MDS_c[i]
        if abs(MDS_o[i]) > sys.float_info.epsilon or abs(MDS_c[i]) > sys.float_info.epsilon:
            count_MDS_oc += 1
...
def saveIOBANDWIDTH(resultr, resultw, title, jobid, program_name, corehour, min_time):
    for i in range(len(resultr)):
        resultr[i] = resultr[i]
        resultw[i] = resultw[i]
    ...
    for xu in xrange(len(resultr)):
        if abs(resultr[xu]) > sys.float_info.epsilon or abs(resultw[xu]) > sys.float_info.epsilon:
...
def savefilename(file_set_open, title, jobid, program_name, corehour, min_time):
    ...
    for xu in xrange(len(file_set_open)):
        if len(file_set_open[xu]) > 0:
            data.append((str(xu), str(file_set_open[xu]), str(len(file_set_open[xu]))))
        else:
            continue
...
def saveprocess(dic, pa, title, min_time, max_time, jobid, program_name, corehour):
    host = dict()
    con = dict()
    count = 0
    for i in range(len(dic)):
...
def save_main(starttime, endtime, jobid, program_name, corehour, title, title_IOmode, host_t):
    global jobid_print
    jobid_print = jobid
    try:
        time1, time2, iplist, min_time, max_time = compute_pre(starttime, endtime, jobid)
    except Exception as e:
        print e
    ...
    if time1[8:10] == time2[8:10]:
        index = [
         time1[0:4] + '.' + time1[5:7] + '.' + time1[8:10]]
    else:
        index = compute_index(time1, time2)
    for lnd in xrange(len(index)):
        if count_ip > 0:
            for c1 in xrange(count_ip):
                results_message_tmp, results_host_tmp = es_search.search(time1, time2, iplist[c1 * node_count:c1 * node_count + node_count], index[lnd], host_t)
                results_message += results_message_tmp
                results_host += results_host_tmp

            if remainder > 0:
                results_message_tmp, results_host_tmp = es_search.search(time1, time2, iplist[count_ip * node_count:count_ip * node_count + remainder], index[lnd], host_t)
        else:
            results_message_tmp, results_host_tmp = es_search.search(time1, time2, iplist, index[lnd], host_t)

    results_message += results_message_tmp
    results_host += results_host_tmp
    try:
        resultr_band, resultw_band, resultr_iops, resultw_iops, resultr_open, resultw_close, resultr_size, resultw_size, dictr, dictw, file_all_count = deal_all_message(results_message, results_host, min_time, max_time)
        del results_message
        del results_host
        gc.collect()
    except Exception as e:
    ...
    saveIOBANDWIDTH(resultr_band, resultw_band, title_IOmode, jobid, program_name, corehour, min_time)
    saveMDS(resultr_open, resultw_close, title_IOmode, jobid, program_name, corehour, min_time)
    saveprocess(dictr, 'r', title_IOmode, min_time, max_time, jobid, program_name, corehour)
    saveprocess(dictw, 'b', title_IOmode, min_time, max_time, jobid, program_name, corehour)
    ...
    sum_resultr, sum_resultw, average_resultr, average_resultw, count_resultr, count_resultw, count_resultrw, average_resultr, average_resultw = sum_IOBANDWIDTH(resultr_band, resultw_band)
    sum_IOPS_r, sum_IOPS_w, count_IOPS_r, count_IOPS_w, count_IOPS_rw, count_IOPS_rw_all, average_IOPS_r, average_IOPS_w = sum_IOPS(resultr_iops, resultw_iops, resultr_band, resultw_band)
    sum_MDS_o, sum_MDS_c, count_MDS_o, count_MDS_c, count_MDS_oc, average_MDS_o, average_MDS_c = sum_MDS(resultr_open, resultw_close)
    return (
     sum_resultr, sum_resultw,
